...
public class ChallengeDAO {
	...
	public boolean makeChallenge(Challenge challenge) {
		try {
			PreparedStatement stmt = conn.prepareStatement(QueryUtil.INSERT_CHALLENGE);
			stmt.setInt(1,challenge.getChallengerId());
			stmt.setInt(2, challenge.getOpponentId());
			stmt.setString(3, challenge.getStatus());
			stmt.setInt(4, challenge.getChallengerScore());
			stmt.setInt(5, challenge.getOpponentScore());
			
			int result = stmt.executeUpdate();
			return result>0;
		}
		catch(SQLException e) {
			LOGGER.warn(new StringBuilder("::: Problem in Insert Challenge :::  "+e.getMessage()+" ::: ").toString());
			return false;
		}
	}
	...
	public List<Challenge> viewChallenges(int userId){
		...
		try {
			...
			while(rs.next()) {
//				challenges.add(new Challenge(rs.getInt("challenger_id"),rs.getInt("challenge_id"),rs.getInt("opponent_id")));
			}
		} catch (Exception e) {
