...
public class NotificationDAO {
	...
	public List<Notification> getAllNotification(int userId){
		...
		try {
			...
			while(rs.next()) {
				notifications.add(new Notification(rs.getInt("notification_id"),userId,rs.getInt("sender_id"),rs.getString("type"),rs.getString("message"),rs.getBoolean("is_read"),rs.getDate("created_at")));
			}
		} catch (SQLException e) {
